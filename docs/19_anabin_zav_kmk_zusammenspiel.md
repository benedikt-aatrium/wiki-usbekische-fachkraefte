...
```mermaid
graph LR
    A[Arbeitgeber bietet Stelle an] --> B[Arbeitgeber reicht ZAV-Antrag ein]
    B --> C[ZAV prüft Arbeitsbedingungen]
    C --> D{Zustimmung?}
    D -->|Ja| E[Kandidat beantragt Visum]
    D -->|Nein| F[Arbeitgeber muss nachbessern]
    F --> B
    E --> G[Einreise und Arbeitsbeginn]
```
...
```mermaid
graph TD
    A[Potenzieller Kandidat in Usbekistan] --> B{Beruf oder Studium?}
    B -->|Berufsausbildung| C[IHK FOSA oder HWK prüfen Anerkennung]
    B -->|Studium/Schule| D[Anabin/KMK prüfen Abschluss]
    C --> E[Anerkennungsbescheid liegt vor]
    D --> F{Gleichwertig?}
    F -->|Ja| E
    F -->|Nein/Teilweise| G[Weitere Qualifikation]
    E --> H[Arbeitgeber stellt ZAV-Antrag]
    G --> H
    H --> I[ZAV erteilt Zustimmung]
    I --> J[Kandidat beantragt Visum]
    J --> K[Einreise nach Deutschland]
    K --> L[Arbeitsbeginn]
```
